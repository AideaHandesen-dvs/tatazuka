...
//   - IO 注入：読み口を opts.readPower で差し替え可能（テストで実 /sys を読まない）。
...
// 既定の読み口：/sys/class/power_supply を走査し、最初のバッテリーの capacity / status と AC の online を返す。
// バッテリーが無い（デスクトップ）・読めなければ null（PE：黙る）。
async function defaultReadPower() {
  let names;
  try {
...
export function createBattery(opts) {
  ...
  const readPower = opts.readPower || defaultReadPower;
