...
const MOOD = {
  ...
  '喜び': { led: '黄',   eyes: '＾ ＾' },
  '怒り': { led: '赤',   eyes: '＞ ＜' },
  '照れ': { led: '桃',   eyes: '／ ／', cheek: true },
};
// act → サーボのジェスチャ（首 pan/tilt と body の上下）。知らない act は無視（§4-1）。
const ACT = {
  'こっちを見る': 'pan 0°, tilt +10°（顔を上げてこちらへ）',
  'うなずく':     'tilt 下→上 ×2',
  '首を振る':     'pan 左→右 ×2',
  '跳ねる':       'body servo 上下 ×2',
};

// v0 メッセージ → 物理駆動コマンド列（純ロジック・IO は log 注入）。
// メソッド名＝メッセージ型名（handle が type で引く）。実機ファームは log を実駆動に差すだけ。
export function makeBody({ log }) {
  let here = true;
  return {
...
export function runStackchan(opts) {
  ...
  return { stop() { closed = true; try { ws && ws.close(); } catch { /* noop */ } }, sendSense, body };
}
