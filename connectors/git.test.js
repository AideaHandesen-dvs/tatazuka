...
//   - poll() は clean↔dirty の遷移だけを返す。初回は基準を取るだけで喋らない
//   - 未コミット数を ctx.n に、リポ名を ctx.repo に乗せる
//   - run が null（git 不在/リポでない）／例外なら null（黙る・PE）
//   - `git -C <repo> status --porcelain` を正しく組む

import test from 'node:test';
import assert from 'node:assert/strict';
import { createGit } from './git.js';

// porcelain 出力を順に返す run スタブ（末尾に達したら最後を返し続ける）。null/'!throw' で失敗を演じる
...
test('`git -C <repo> status --porcelain` を組む（末尾スラッシュは repo 名で正規化）', async () => {
  const r = gitRun([CLEAN]);
  const src = createGit({ run: r, env: { TZ_GIT_REPO: '/repo/foo/' } });
  await src.poll();
  assert.deepEqual(r.calls[0], { cmd: 'git', args: ['-C', '/repo/foo/', 'status', '--porcelain'] });
});
