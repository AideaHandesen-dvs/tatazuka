...
### 受動の口（talk・protocol §5-4）— ユーザーが佇かに話しかける

ユーザー→佇かの自由文（`{type:'talk', data:{text}}`）。behavior の `onTalk` が受け、**返事は普通の
`say`**（server→client の語彙は増えない）。人格層の床/格上げの型がそのまま効く：

- **ルール表は相槌の床**（`TABLES['talk']`）。内容は理解できないが「ふーん。…で？」と受ける＝
  LLM 無しでも・遅くても・壊れても、話しかけて無視されることはない。
- **LLM persona は会話に格上げ**。`ctx.text`（ユーザーの言葉）と `ctx.history`（直近のやり取り）を
  user プロンプトに織り込み、キャラの声色で返事を生成する。**締めの指示は talk だけ「相手への返事」**
  ——茶々の語り口（「この状況での一言を出せ」）のまま会話に答えさせると、質問を無視した
  「状況の感想」になる（3B 実測：「寒くない？」→「寒くないのはうれしいな」）。最も直近の指示が
  一番効く位置（出力契約を user に置くのと同じ知見）で返事モードに切り替えると噛み合う。
- **「間」は server が刻む**（§4-1）：受信した瞬間に `motion:'うなずく'` を返してから生成を待つ。
  最悪ケース（LLM 8 秒タイムアウト→床）でも、頷きがあるので「聞いている」に読める。
- **会話の短期記憶（`talkmemory.js`）**：直近 **6 往復**をプロセスに一つ持つ（reunion と同じ注入の
  継ぎ目。serve.js が作って全部屋で共有）。佇かは一人なので**部屋を移っても・WS が瞬断しても会話が
  続く**。ディスクに書かない＝再起動で消える。記憶に残すのは**実際に返事が届いた往復だけ**
  （生成中に切断したら覚えない）。
- **入力の防御**（§5-4）：trim・空は黙って無視・500 字超は頭だけ読む。ガードは behavior に置く
  （ws.js は型非依存のまま）。
- **prompt injection への構え**：ユーザーの自由文は **LLM の user プロンプトにしか入れない**。
  system（キャラ＋出力契約）は不変で、出力は parse＋sanitize＋mood 語彙検証＋ルール表の床が
  構造的に受ける。実行口は存在しない（README §7-1 soft の構造担保と同型）。
- **空き部屋に話しかけると佇かが移ってくる**（hub が sense と同じ移動の蛇口として解釈・挨拶なしで返事）。
- 既知の割り切り：連打や複数部屋からの同時 talk は生成が並び、完了順に `say` が上書きする（v0 許容）。
...
| **mqtthumidity** | `TZ_MQTT_URL`（ブローカー）＋ `TZ_MQTT_HUMIDITY`（topic）（＋`TZ_MQTT_HUMIDITY_PATH` 取り出しパス・Tasmota は `AM2301.Humidity`・`TZ_HUMIDITY_LOW/HIGH` 共有） | **非 HA**：MQTT の湿度 topic を室内湿度の快適帯に（humidity の別トランスポート・両側帯） | `humidity.dry` / `humidity.humid` / `humidity.ok` |
