...
### 配布メモ（層②の Windows 版）— .exe / SmartScreen と「ダブルクリック」の現実

mac の `.app`/Gatekeeper 節（上）と対になる Windows の現実。「インストーラをダブルクリックしたら一発で導入」を
素直に作れるか：

- **まず大前提：「ダブルクリックでサーバ窓が開く」アプリにはしない**。佇かは前面アプリでなく**裏で常駐する
  サーバ**で、表示は別端末のブラウザに逃がす設計（CLAUDE.md）＝**開く窓がそもそも無い**。だから日常の起動は
  上の **Task Scheduler ユニット**（ログオンで黙って常駐）が本質に合う＝Mac の LaunchAgent / Linux の systemd と同じ。
  ダブルクリックが効いてほしいのは「日々の起動」でなく「**一度きりの導入**」の方。
- **`.ps1` はダブルクリックで実行できない**（既定でエディタが開く＝Windows のセキュリティ仕様）。一発導入を
  ダブルクリックで起こすなら **`.exe`（Inno Setup / NSIS）** か、最低限 `.cmd`／`.lnk`。中身は本節の手順
  （node 配置・タスク登録・証明書）をなぞるだけ＝**コンパイル環境の話ではない**。node は `.exe` に同梱すれば
  ユーザーは何も入れなくていい（Mac の `Resources/` 同梱と同じ）。
- **“警告なし”の壁は SmartScreen**（Gatekeeper の対）。署名の無い `.exe` は **「Windows によって PC が保護
  されました」**で止まる。回避は二択：①ユーザーが **「詳細情報 → 実行」**の一手間（摩擦は残るが無料）。
  ②**コード署名証明書**で署名し SmartScreen の評判を貯める（証明書は有償・OV/EV で相場が違う。EV は即時、
  OV は評判が貯まるまで暫く警告が出る）。「摩擦ゼロ配布」だけが署名待ち＝Mac の公証($99)と同じ構図。
- **管理者昇格が一度要る**：FW 開放（LAN の他端末から届かせる）と——sshd を入れた時のように——環境次第で
  一部の仕込みに昇格が要る。タスク自体は LeastPrivilege で動く（landed 済）が、**LAN 開放の FW 例外付与は admin 一回**。
  `.exe` インストーラなら UAC で一度だけ昇格すれば済む。
- **佇か固有の捻りは Mac と同じ**：表示は別端末のブラウザなので、サーバの自己署名証明書を**見る側の端末に
  信頼させる**一手が要る——普通の Windows アプリには無い、佇か固有の導入課題（OS 非依存）。

つまり層②の Windows 版も「`.exe` を作れるか」ではなく「**SmartScreen（署名）＋常駐の仕込み（タスク登録は
landed 済）＋FW 開放の昇格＋他端末の証明書信頼**」が本体。①（Task Scheduler ユニット）が landed でも、ここは
未着手＝Mac と同じ温度。
